...
class Temsys():
    def __init__ (self):
        ...
        try:
            ...
            print("Error al conectar con servidor mqtt " + "localhost")
            self.mqttserver = 1
    ...
    while True:
        ...
        for s in temsys.sensors:
            ...
            if data == "" or data == None:
                ...
                topic = temsys.mqttc.topic + data[0]['sensor'] + "/" + data[0]['type']
                temsys.mqttc.publish(topic, data[0]['value'])
                logging.info("Sensor: %s tipo: %s valor: %s -> topic publicado: %s" % (data[0]['sensor'], data[0]['type'].strip('\n'), data[0]['value'], topic))
                topic = temsys.mqttc.topic + data[1]['sensor'] + "/" + data[1]['type']
                temsys.mqttc.publish(topic, data[1]['value'])
